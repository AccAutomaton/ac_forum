...
const getArticleList = async () => {
  const data = await getArtistCollectionListByUid(artistId, currentPageNumber.value, currentPageSize.value);
  if (data !== null) {
...
const emit = defineEmits(["decreaseCollections"]);

const onClickUnCollectButton = async (index, articleId) => {
  const data = await unCollectArticle(articleId);
  if (data !== null) {
    articleList.value.splice(index, 1);
    emit("decreaseCollections");
  }
}
</script>
...
<template>
  <template v-if="articleList.length === 0">
    <el-empty description="无收藏内容" :image-size="150"/>
  </template>
  <template v-else>
              ...
              <el-col v-if="isSelfLivingRoom" :span="2">
                <el-row align="middle" justify="center" style="margin-right: -15px">
                  <el-popconfirm confirm-button-text="确认" cancel-button-text="取消"
                                 ...
                                 title="取消收藏该文章？"
                                 offset="20"
                                 @confirm="onClickUnCollectButton(index, article['id'])"
                                 :hide-after="0"
                                 width="200px"
